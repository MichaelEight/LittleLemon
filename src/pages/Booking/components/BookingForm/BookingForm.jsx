...
export const BookingForm = ({ onSubmit }) => {
  const {
    state: { availableTimes, formData, formErrors, isDirty, occasions_list, spots_list },
    dispatch,
  } = useForm() ?? {
    state: {
      availableTimes: [],
  ...
  return (
    ...
    >
      ...
      <Select
        ...
        id="spot"
        name="spot"
        placeholder="Choose the spot"
        dirtyPlaceholder="No preferred spot"
        options={spots_list}
        onFocus={onFocus}
        onBlur={onBlur}
        onChange={handleChange}
        value={formData?.spot ?? ''}
        errors={formErrors.spot}
      />
